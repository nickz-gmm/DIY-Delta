...
fn parse_packet(buf: &[u8], expected_format: u16) -> Option<TelemetrySample> {
    ...
    if hdr.packet_format != expected_format { /* accept cross-year for convenience */ }

    static mut STATE: Option<PlayerState> = None;
    unsafe {
        if STATE.is_none() { STATE = Some(PlayerState::default()); }
        ...
        match hdr.packet_id {
            PACKET_MOTION => {
                ...
                let car_stride = 60*4 + 4*4; // approximate minimal fields; spec object is larger; we only skip by 1464 for all cars usually
